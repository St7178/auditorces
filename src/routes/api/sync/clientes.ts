...
export const Route = createFileRoute("/api/sync/clientes")({
    server: {
        handlers: {
            GET: async () => {
                let data: typeof CLIENTES = CLIENTES;
                try {
            ...
            POST: async ({ request }) => {
                try {
                    ...
                    if (!Array.isArray(body)) {
                        return new Response(JSON.stringify({ error: "El body debe ser un array de clientes" }), { status: 400, headers: { "Content-Type": "application/json" } });
                    }
