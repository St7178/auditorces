...
export interface CoverflowSlide {
    /** Contenido de la cara de la tarjeta (sin imágenes — solo fondo oscuro/claro). */
    face: React.ReactNode;
    /** Fondo de la cara: "dark"/"light" alternan por estética; "danger" fuerza énfasis (p.ej. una
     *  alerta real) sin importar en qué posición cae. */
...
export interface CoverflowCarouselProps {
    ...
    /** Se llama cada vez que la tarjeta central cambia (por arrastre, flechas o paginación). */
    onSelectChange?: (index: number) => void;
}

const TONE_CLASS: Record<NonNullable<CoverflowSlide["tone"]>, string> = {
    dark: "bg-slate-900 text-white",
    light: "bg-slate-50 text-slate-900 border border-border/60",
    danger: "bg-red-950 text-red-50",
};
                        ...
                        {slides.map((slide, index) => (
                            <div
                                ...
                                className={cn(
                                    ...
                                    TONE_CLASS[slide.tone ?? "light"],
                                    cardClassName,
                                )}
                                style={{ width: "var(--cf-card)" }}
                            >
                                <div className="flex h-full w-full items-center justify-center p-4">{slide.face}</div>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {showCaption && active?.title && (
                ...
                >
                    ...
                    {active.meta && active.meta.length > 0 && (
                        <dl className="mt-6 w-full max-w-sm text-[12px]">
                            {active.meta.map((row, i) => (
                                <div key={`${row.label}-${i}`} className="flex justify-between gap-3 border-b border-border/40 py-[6px] last:border-0">
